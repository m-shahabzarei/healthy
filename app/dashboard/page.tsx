...
export default function DashboardPage() {
  ...
  return <AppShell active="home">
    <div className="dashboard-page app-container">
      <header className="dashboard-header"><div><p className="eyebrow"><span className="eyebrow-line" /> {formatDate(locale, new Date(), { weekday: 'long', day: 'numeric', month: 'long' })}</p><h1 className="page-title">{t('Good morning, {name}.', { name: user.displayName })}</h1><p className="page-subtitle">{t('Come back to your pace. You do not have to solve everything today.')}</p></div><div className="streak-chip"><Clock3 size={16} aria-hidden="true" /><strong>{formatNumber(locale, streak, { maximumFractionDigits: 0 })}</strong><span>{t('day streak')}</span></div></header>
